...
def initiate_pvp(vk, attacker, target_id):
    ...
    if target["current_sec"] != attacker["current_sec"]:
        send(vk, attacker["vk_id"], "Игрок ушёл из сектора.", None); return
...
def process_pvp_action(vk, p, action, sid):
    ...
    if action == "attack":
        ...
        if random.random() < 0.5:
            ...
            send(vk, p["vk_id"], "🏃 Сбежал!", kb([[("◀️ В лагерь", "secondary")]]))
            send(vk, defender_id, f"🏃 {p['name']} сбежал. Ты победил!", kb([[("◀️ В лагерь", "secondary")]]))
            return
...
def initiate_trade(vk, initiator, target_id):
    ...
    if target["current_sec"] != initiator["current_sec"]:
        send(vk, initiator["vk_id"], "Игрок ушёл.", None); return
...
def handle_message(vk, vk_id, text):
    ...
    if text.startswith("⚔️ Принять бой:"):
        ...
        if not s or s["state"] != "pending" or s["expires_at"] < int(time.time()):
            send(vk, p["vk_id"], "Время вышло.", kb([[("◀️ В лагерь", "secondary")]]))
            p["state"] = "main"; p["temp"] = ""; save_player(p)
            return
        if s["target"] != p["vk_id"]:
