...
def screen_main(vk, p):
    ...
    quest_line = ""
    if p["active_quest"] and p["active_quest"] in QUESTS:
        q = QUESTS[p["active_quest"]]
        quest_line = f"\n📋 {q['name']}: {p['quest_progress']}/{q['goal_count']}"
    text = (
        f"👤 {p['name']} | Ур.{p['level']} | {p['tokens']} жетонов\n"
        f"❤️ {p['hp']}/{p['max_hp']} {hp_bar}\n"
        f"🌡 {zone['temp']}°C | 📍 {zone['name']}{quest_line}\n"
        f"────────────────\n"
        f"{zone['description']}"
    )
    keyboard = kb([
...
def handle_message(vk, vk_id, text):
    ...
    if state == "enter_name":
        ...
        if text.startswith(("📊", "🗺", "🏪", "🎒", "💬", "📋", "⚔️", "◀️")):
            send(vk, vk_id, "Сначала введи своё имя, сталкер:")
            return
        name = text[:20].strip()
        if len(name) < 2:
    ...
    # Quest accept
        ...
        if not qid:
            ...
            return
        if p["active_quest"]:
            send(vk, vk_id, "У тебя уже есть активный квест. Сначала заверши его.", kb([[("◀️ Назад", "secondary")]]))
            return
        p["active_quest"] = qid
        p["quest_progress"] = 0
        save_player(p)
