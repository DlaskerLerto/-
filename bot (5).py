...
def screen_main(vk, p):
    ...
    # Убежище: пассивная регенерация (1% макс HP в минуту)
    sec_now = ALL_SECTORS.get(p["current_sec"], {})
    if sec_now.get("type") in ("shelter", "faction_base") and p["hp"] < p["max_hp"]:
        now_ts = int(time.time())
        last = p.get("last_seen", now_ts)
        elapsed = max(0, now_ts - last)
        regen_per_sec = p["max_hp"] / 120.0  # полное восст за 2 минуты
        regen = max(0, int(elapsed * regen_per_sec))
        if regen > 0:
...
def create_session(initiator_id, target_id, stype, data=None):
    ...
    con = sqlite3.connect(DB_PATH)
    c = con.cursor()
    # expires_at = 0 пока, обновим после отправки уведомления
    c.execute(
        ...
        (sid, stype, initiator_id, target_id, "pending",
         json.dumps(data or {}), now, 0)
    )
    con.commit(); con.close()
    return sid

def activate_session_timer(sid):
    """Запускает таймер сессии — вызывать ПОСЛЕ отправки уведомления."""
    now = int(time.time())
    con = sqlite3.connect(DB_PATH)
    c = con.cursor()
    c.execute("UPDATE sessions SET expires_at=? WHERE session_id=?",
              (now + SESSION_TIMEOUT, sid))
    con.commit(); con.close()
...
def initiate_pvp(vk, attacker, target_id):
    ...
    # Запускаем таймер только после доставки уведомления
    activate_session_timer(sid)
...
def start_pvp_battle(vk, sid):
    ...
    # Бонус защиты на своей базе группировки
    a_sec = ALL_SECTORS.get(a["current_sec"], {})
    b_sec = ALL_SECTORS.get(b["current_sec"], {})
    a_home = 20 if (a_sec.get("type") == "faction_base" and a_sec.get("faction") == a["faction"]) else 0
    b_home = 20 if (b_sec.get("type") == "faction_base" and b_sec.get("faction") == b["faction"]) else 0
...
def process_pvp_action(vk, p, action, sid):
    ...
    # Бонус защиты на своей базе
    def_home = bd.get("b_home", 0) if is_a else bd.get("a_home", 0)
    def_ = max(0, d_stats["defense"] + def_home)
    log_line = ""
...
def start_battle(vk, p, enemy_data=None):
    ...
    if enemy_data is None:
        ...
        # Гарнизон базы — обычная сложность для PvE
        # (усиление только для PvP происходит отдельно)
...
def _send_battle_buttons(vk, p, text):
    ...
    special_on_cd = bs.get("special_used", False) if isinstance(bs, dict) else False
    ...
    if special_name:
        if special_on_cd:
            btns.append([(f"⏳ {special_name} [КД]", "secondary")])
        else:
...
def process_battle_action(vk, p, action):
    ...
    if enemy["hp"] <= 0:
        ...
        arts_found = []
        sec = ALL_SECTORS.get(p["current_sec"], {})
        enemy_name_raw = enemy["name"]
        # Мутанты не дропают артефакты
        mutants = {"Одичавшая собака","Кровосос","Псевдособака","Псевдогигант",
                   "Контролёр","Химера","Полтергейст","Излом","Вечный страж"}
        is_mutant = any(m in enemy_name_raw for m in mutants)
        sec_danger = sec.get("danger", 1)
        if not is_mutant and sec_danger >= 3:
            # Только в опасных зонах (3+), редкие артефакты только в зонах 5+
            art_pool = [a for a, data in ARTIFACTS.items()
                        if data["cost"] <= 250 or sec_danger >= 5]
            base_chance = 0.04 + sec_danger * 0.02  # 10-16% в зоне 3-6
            for _ in range(sec.get("artifacts", 0)):
                if random.random() < base_chance:
                    arts_found.append(random.choice(art_pool))
        for a in arts_found:
...
def screen_sell(vk, p):
    """Экран продажи предметов торговцу."""
    loc = LOCATIONS.get(p["current_loc"], {})
    quality = loc.get("trader_quality", 0)
    if quality == 0:
        send(vk, p["vk_id"], "Здесь нет торговцев.", None)
        screen_main(vk, p); return
    ...
    for item, qty in p["inventory"].items():
        if qty > 0:
            ...
            if item in WEAPONS and WEAPONS[item]["cost"] > 0:
                sellable[item] = (WEAPONS[item]["cost"] // 2, qty)
            elif item in ARMORS and ARMORS[item]["cost"] > 0:
                sellable[item] = (ARMORS[item]["cost"] // 2, qty)
            elif item in MEDKITS:
                sellable[item] = (MEDKITS[item]["cost"] // 2, qty)
            elif item in ARTIFACTS:
                sellable[item] = (ARTIFACTS[item]["cost"] // 2, qty)

    if not sellable:
        send(vk, p["vk_id"], "Нечего продавать.", None)
        screen_trader(vk, p); return

    text = f"💰 ПРОДАЖА\nЖетоны: {p['tokens']}\n══════════════════════════════\n"
    text += "Торговец даёт 50% от цены:\n\n"
    btns = []
    for item, (price, qty) in list(sellable.items())[:10]:
        text += f"• {item} ×{qty} → {price} жт каждый\n"
        btns.append([(f"Продать: {item} ({price}жт)", "positive")])
    btns.append([("◀️ Назад", "secondary")])
    p["temp"] = "sell_item"
    save_player(p)
    send(vk, p["vk_id"], text, kb(btns))
...
def go_to_shelter(vk, p):
    """Переключает игрока в режим убежища в текущей локации — не меняет локацию."""
    loc = LOCATIONS.get(p["current_loc"], {})
    shelter_sec = None
    # Ищем убежище или базу фракции в ТЕКУЩЕЙ локации
    ...
    if not shelter_sec:
        for sec_id, sec in loc.get("sectors", {}).items():
            if sec.get("type") == "faction_base" and sec.get("faction") == p["faction"]:
                shelter_sec = sec_id
                break
    # Если в текущей локации нет убежища — показываем что укрываемся на месте
    if shelter_sec:
        p["current_sec"] = shelter_sec
    p["cold_stacks"] = 0  # холод сбрасывается
    save_player(p)
    sec = ALL_SECTORS.get(p["current_sec"], {})
    send(vk, p["vk_id"],
         f"🏠 УБЕЖИЩЕ\n══════════════════════════════\n"
         ...
         f"💚 HP восстанавливается пока ты здесь.\n"
         f"👁 Другие игроки тебя не видят.\n\n"
         f"Нажми «В бой» — вернёшься в тот же сектор.",
         None)
    screen_main(vk, p)
...
def handle_message(vk, vk_id, text):
    ...
    if text == "💰 Продать предмет":
        screen_sell(vk, p); return
    ...
    # ── Продажа предмета ──
    if temp == "sell_item" and text.startswith("Продать: "):
        # Формат: "Продать: ИмяПредмета (цена жт)"
        item_part = text[9:]
        item_name = item_part.split(" (")[0]
        price = 0
        for catalog in (WEAPONS, ARMORS, MEDKITS, ARTIFACTS):
            if item_name in catalog:
                cost = catalog[item_name].get("cost", 0) if isinstance(catalog[item_name], dict) else 0
                price = cost // 2
                break
        if price > 0 and p["inventory"].get(item_name, 0) > 0:
            p["inventory"][item_name] -= 1
            p["tokens"] += price
            p["temp"] = ""
            save_player(p)
            send(vk, p["vk_id"], f"✅ Продано: {item_name} за {price} жт.\nЖетоны: {p['tokens']}", None)
            screen_trader(vk, p)
        else:
            send(vk, p["vk_id"], "Предмет не найден.", None)
            screen_sell(vk, p)
        return
